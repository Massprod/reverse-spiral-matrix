import numpy


def create_square_matrix(row_length: int) -> list[list[int]]:
    """Creating square matrix with given row length."""
    matrix_length = row_length ** 2
    square_matrix = [x for x in range(1, matrix_length + 1)]
    square_matrix = numpy.asarray(square_matrix).reshape(row_length, row_length)
    return square_matrix


def reverse_spiral_read(matrix: list[list[int]]) -> list[int]:
    """Creating list of values in reverse_spiral_order of the given square matrix."""
    row_length = len(matrix[0])
    reverse_spiral = []
    steps = 1  # number of turns
    x, y = -1, 0  # starting indexes x - row, y - column (start from -1 to include 0, 0)
    s_row, s_column = 1, 0  # row/column step length
    while steps <= row_length ** 2:
        if 0 <= x + s_row < row_length and 0 <= y + s_column < row_length \
                and matrix[x + s_row][y + s_column] != -1:
            x += s_row
            y += s_column
            reverse_spiral.append(matrix[x][y])
            steps += 1
            matrix[x][y] = -1
        else:
            if s_column == 1:
                s_column = 0
                s_row = 1
            elif s_row == 1:
                s_row = 0
                s_column = -1
            elif s_column == -1:
                s_column = 0
                s_row = -1
            elif s_row == -1:
                s_row = 0
                s_column = 1
    return reverse_spiral
